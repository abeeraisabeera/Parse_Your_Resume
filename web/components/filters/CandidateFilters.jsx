...
export function CandidateFilters({
  ...
  function toggleSkill(skillValue) {
    ...
    if (selected.has(skillValue.toLowerCase())) {
      selected.delete(skillValue.toLowerCase());
    } else {
      selected.add(skillValue.toLowerCase());
    }

    const nextSkills = skillGroups
      .flatMap((group) => group.options)
      .map(([value]) => value)
      .filter((value) => selected.has(String(value).toLowerCase()));
    setFilter("skillFilter", nextSkills.join(", "));
  }
      ...
      {expanded ? (
        <div className="filterStack">
          <div className="filterSection">
            ...
            <div className="filterGrid primaryFilters">
          ...
          <div className="filterSection skillFilterSection">
            <div className="filterSectionHeader">
              <h3>Skills</h3>
              <span className="sliderValue">
                {selectedSkills.length ? `${selectedSkills.length} selected` : "Select one or more skills"}
              </span>
            </div>
            <div className="skillChecklist">
              {skillGroups.map((group) => (
                <div className="skillGroup" key={group.label}>
                  <strong>{group.label}</strong>
                  <div className="skillCheckGrid">
                    {group.options.map(([value, label]) => (
                      <label className="skillCheck" key={value}>
                        <input
                          type="checkbox"
                          checked={selectedSkills.some((skill) => skill.toLowerCase() === String(value).toLowerCase())}
